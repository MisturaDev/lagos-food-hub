...
export default function BeneficiaryDashboard() {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-10">
      <PageHeader
        title="Beneficiary Workspace"
        description="Share your needs so coordinators can match nearby surplus food."
      />
          ...
          <Card title="Matching Queue" description={`${openMatches.length} open match${openMatches.length === 1 ? "" : "es"}`}>
            {openMatches.length === 0 ? (
              <EmptyState
                ...
                action={
                  <Link href="/matches" className="text-xs font-semibold text-[#166534] hover:underline">
                    Open Match Center
                  </Link>
                }
              />
